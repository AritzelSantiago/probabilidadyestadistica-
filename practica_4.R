...
#--------------------------------------
#    Exploracion
#dim es la dimension, numero de observaciones y variables
#colnames es el nombre de las variables
#str da el tipo de variables
#anyNA si hay datos faltantes
#summary nos da las medidas de tendencia central
...
color=c("lightpink3","deepskyblue4")
...
# 1.- Creación de un vector de color
color=c("bisque", "lavenderblush1", "deepskyblue3")

# 2.- Creación del gráfico
GB1<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "blue4", fill=color)+
...
HG<-ggplot(penguins, aes(x=largo_aleta_mm))+
  geom_histogram(col="blue4", fill="coral3")+
  ggtitle("Histograma")+
...
GD<-ggplot(penguins, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  ...
  scale_color_manual(values=c("violet","coral1","hotpink3"))+
